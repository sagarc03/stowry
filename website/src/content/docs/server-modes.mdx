...
- Reads are public: `auth.read: private` is rejected at startup, and `auth.write` is ignored with a warning
- Put files in the storage directory and run `stowry populate`; static and spa route no writes
...
```bash
# Record files already in the storage directory
stowry populate   # after copying ./dist into the storage directory
...
The config file is not shell-expanded, so `${VAR}` in it is a literal string.
To take the mode from the environment, set `STOWRY_SERVER_MODE` instead.
