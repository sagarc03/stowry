...
```go
import "github.com/sagarc03/stowry/database"
...
Migrations are code-based, not SQL files. They run automatically when using `database.Connect()`:

```go
// Migrations run automatically via Connect()
repo, cleanup, err := database.Connect(ctx, cfg)
// Schema is migrated and validated automatically
```
